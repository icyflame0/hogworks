...
// ── Modal outside AdminPanel so it never re-renders the list ──
const Modal = ({ show, onClose, children }) => !show ? null : (
  <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.78)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: '16px' }}>
    <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: '420px', background: 'linear-gradient(145deg,#2a0d42,#170826)', border: '1px solid rgba(201,168,76,0.28)', borderRadius: '22px', padding: '22px', animation: 'slideUp .3s ease-out' }}>
      {children}
    </div>
  </div>
);

// ── Add Guest Form as its own component with its own state ──
const AddGuestForm = ({ onClose, onSuccess }) => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      onSuccess(res.data.guest);
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to add guest');
    } finally { setSaving(false); }
  };

  const labelStyle = { display: 'block', color: 'rgba(201,168,76,0.52)', fontSize: '10px', fontFamily: "'Cinzel',serif", letterSpacing: '.16em', textTransform: 'uppercase', marginBottom: '6px' };

  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '15px' }}>
        <h3 style={{ fontFamily: "'Cinzel',serif", fontSize: '14px', fontWeight: 600, color: '#fbbf24', margin: 0 }}>✨ Enroll New Guest</h3>
        <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'rgba(201,168,76,0.45)', fontSize: '24px', cursor: 'pointer', lineHeight: 1 }}>×</button>
      </div>
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '60vh', overflowY: 'auto', paddingRight: '2px' }}>
        <div><label style={labelStyle}>Full Name *</label><input type="text" placeholder="Harry Potter" value={form.name} onChange={update('name')} required /></div>
        <div><label style={labelStyle}>Phone</label><input type="tel" placeholder="+91 XXXXX XXXXX" value={form.phone} onChange={update('phone')} /></div>
        <div><label style={labelStyle}>Email</label><input type="email" placeholder="guest@example.com" value={form.email} onChange={update('email')} /></div>
        <div><label style={labelStyle}>Check-in Date & Time *</label><input type="datetime-local" value={form.checkInDate} onChange={update('checkInDate')} required /></div>
        <div><label style={labelStyle}>Check-out Date & Time *</label><input type="datetime-local" value={form.checkOutDate} onChange={update('checkOutDate')} required /></div>
        <div><label style={labelStyle}>Special Notes</label><textarea value={form.specialNotes} onChange={update('specialNotes')} placeholder="Any special requirements..." rows={2} style={{ resize: 'none' }} /></div>
        ...
        <button type="submit" disabled={saving} style={{ padding: '13px', border: 'none', borderRadius: '12px', fontFamily: "'Cinzel',serif", fontSize: '13px', cursor: 'pointer', opacity: saving ? 0.6 : 1, background: 'linear-gradient(135deg,#8b5e00,#c9a84c,#f0c060)', color: '#1a0900', fontWeight: 700 }}>
...
export default function AdminPanel() {
  ...
  const handleAddSuccess = (guest) => {
    setNewCode(guest.accessCode);
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(170deg,#050010,#190929)' }}>
      ...
      <div style={{ padding: '16px' }}>
        ...
        {loading ? (
          ...
          const s = getStatus(g);
          return (
            <div key={g._id} style={{ borderRadius: '16px', padding: '15px', marginBottom: '9px', background: 'rgba(12,4,26,0.98)', border: '1px solid rgba(201,168,76,0.09)' }}>
      ...
      <Modal show={showAdd} onClose={() => setShowAdd(false)}>
        <AddGuestForm onClose={() => setShowAdd(false)} onSuccess={handleAddSuccess} />
      </Modal>
